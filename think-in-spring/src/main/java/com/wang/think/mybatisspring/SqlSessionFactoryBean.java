...
import com.wang.think.mybatisonly.TestMapper;
...
/**
 * SqlSessionFactoryBean 是 MyBatis-Spring 源码解析的入口。封装了 MyBatis 中的实现。
 * SqlSessionFactoryBean 类 实现了 InitializingBean 接口，因此一定也实现了{@link InitializingBean#afterPropertiesSet()}
 * 方法；并且实现了 FactoryBean 接口，那么通过 getBean 方法获取 bean 实例时。其实是从此类的 {@link #getObject()} 方法中返回的实例。
 * 首先，我们从 {@link #afterPropertiesSet()}方法作为入口。
 * <p>
 * 为了使用 MyBatis 功能，{@link https://github.com/cuiweiman/wang-wen-jun.git#com.wang.think.mybatisspring}示例中的 Spring 配置
 * 提供了两个 bean，一个是 SqlSessionFactoryBean，即本类实例，另一个是 {@link org.mybatis.spring.mapper.MapperFactoryBean} 类型的 bean。
 * <p>
 * 在单独使用 MyBatis 框架时{@link https://github.com/cuiweiman/wang-wen-jun.git#com.wang.think.mybatisonly.TestMapper}，调用数据库
 * 接口的方式是：
 * SqlSession sqlSession = sqlSessionFactory.openSession();
 * UserPoMapper mapper = sqlSession.getMapper(UserPoMapper.class);
 * 而对于 Spring-MyBatis 中的创建方式是：
 * ApplicationContext context = new ClassPathXmlApplicationContext("test/mybatis-spring/spring.xml");
 * UserPoMapper userMapperPo = (UserPoMapper) context.getBean("userMapperPo");
 * Spring 创建 UserPoMapper 的方式和MyBatis 单独使用时完成的功能一样，因此可以推断，MyBatis-Spring 在 bean 的创建过程中，一定是使用
 * 了 MyBatis 的原生方法{@link org.apache.ibatis.session.SqlSession#getMapper(Class)}进行了再一次封装。
 * <p>
 * 因此，前往 {@link org.mybatis.spring.mapper.MapperFactoryBean} 源码进行分析。
 *
 * <p>
